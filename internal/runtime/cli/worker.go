...
// BusWorker connects a cli.Runner to the Pepper message bus.
// It subscribes to group topics and executes the CLI tool for each request,
// publishing res/err messages back on pepper.res.{origin_id}.
//
// One BusWorker is created per registered CLI capability by bootRuntime.
...
// NewBusWorker creates a BusWorker for a CLI capability.
// blobMgr is used to register temp-file outputs as zero-copy BlobRefs instead
...
func NewBusWorker(
	...
	return &BusWorker{
		id:        workerID,
		capName:   capName,
		runner:    NewRunner(spec, "", blobMgr, blobTTL),
		groups:    groups,
		bus:       b,
		codec:     c,
		blobs:     blobMgr,
		logger:    log.Namespace("cli:" + workerID),
		startedAt: time.Now(),
	}
}
...
func (w *BusWorker) Start(ctx context.Context) error {
	for _, g := range w.groups {
		...
		if err != nil {
			return fmt.Errorf("cli.BusWorker %s: subscribe push %q: %w", w.id, g, err)
		}
		go w.serveLoop(ctx, pushCh)

		pubCh, err := w.bus.Subscribe(ctx, bus.TopicPub(g))
		if err != nil {
			return fmt.Errorf("cli.BusWorker %s: subscribe pub %q: %w", w.id, g, err)
		}
...
func (w *BusWorker) handle(ctx context.Context, msg bus.Message) {
	...
	if env.DeadlineMs > 0 && time.Now().UnixMilli() > env.DeadlineMs {
		w.sendErr(env, envelope.ErrDeadlineExceeded, "deadline exceeded")
		return
	}
	...
	for {
		...
		case <-ticker.C:
			...
			ping := map[string]any{
				...
				"load":            uint8(load),
				"groups":          w.groups,
				"requests_served": w.requestsServed.Load(),
